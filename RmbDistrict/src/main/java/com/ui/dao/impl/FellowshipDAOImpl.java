...
public class FellowshipDAOImpl implements FellowshipDAO {
	...
	public String addFellowship(Fellowship m) {
      ...
      String sql = "insert into fellowship (fellowship_name,status) values (?,?)";
	...
	public List<Fellowship> getAllFellowship() {
		  logger.info("+++++ GET Fellowship NAME +++++");
		    ...
		    String s = "y";
		    String sql = "select fellowship_id ,fellowship_name from fellowship where status=? ";
		    Connection conn = null;
		    try {
		        conn = dataSource.getConnection();
		        PreparedStatement ps = conn.prepareStatement(sql);
	...
	public Fellowship getFellowshipById(int fellowship_id) {
		Fellowship m = null;
		String sql = "SELECT fellowship_id,fellowship_name FROM fellowship where fellowship_id=?";
	...
	public String editFellowship(Fellowship m) {
		  logger.info("+++++ EDIT Fellowship +++++");
		    String sql = "update fellowship set fellowship_name=?, status=?  where fellowship_id=?";
		    Connection conn = null;
		    try {
		        conn = dataSource.getConnection();
		        PreparedStatement ps = conn.prepareStatement(sql);
