...
/*	
	...
	public Members getLastMemberSequenceByCategory(HttpServletRequest request, String userName,String password) {
	...
	@RequestMapping(value = "Frontlogout", method = RequestMethod.POST)
...
*/	
	...
	public String login(HttpSession session, String userName, String password) {
		...
		if (user != null) {
			...
			session.setAttribute("adminid", String.valueOf(user.getUserId()));
			//session.setAttribute("loginid", user.getUserId());
			session.setAttribute("adminshowname", user.getUserName());
	...
	public String frontLogin(HttpSession session, String userName, String password, String loggedin, HttpServletResponse response) {
		...
		BoardOfDirectors boardOfDirectors = logindao.getBoardOfDirectorsInfo(members.getMemberId());
		if (boardOfDirectors != null) {
			session.setMaxInactiveInterval(30 * 60); // Set Session Time Out			
			session.setAttribute("boardmemberid", String.valueOf(boardOfDirectors.getMemberId()));
			session.setAttribute("boardmemberfamilyid", String.valueOf(boardOfDirectors.getMemberFamilyId()));	
			session.setAttribute("loginid", boardOfDirectors.getMemberId());
		} else {
